...
from Eye_Detector_Module import EyeDetector as EyeDet
...
def app():
    ...
    while True:
        ...
        placeholder.image(output_frame, channels="RGB")
        ...
        if delta_time >= time_lim:
            ...
            if len(faces) > 0:  # process the frame only if at least a face is found
                ...
                if looking_away:
                    cv2.putText(frame, "LOOKING AWAY!", (10, 320),
                                cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1, cv2.LINE_AA)
                    text_result += "LOOKING AWAY!" + "\n"
                if distracted:
                    cv2.putText(frame, "DISTRACTED!", (10, 340),
                                cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 1, cv2.LINE_AA)
                    text_result += "DISTRACTED!" + "\n"
            ...
            text_result += pred if pred else ' '
            output_placeholder.text(text_result)
